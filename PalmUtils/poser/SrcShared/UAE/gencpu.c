...
static void gen_opcode (uae_u32 opcode)
{
	...
	need_endlabel = 1;
#if PALM_PERF
	// ••• still missing a read cycle sometimes.  Weird.  Don't know the address read from, so can't compute waits...
	...
	need_endlabel = 1;
#if PALM_PERF
	// ••• still missing a read cycle somewhere, when counter expires
